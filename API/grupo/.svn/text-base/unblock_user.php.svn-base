...
if ($_SESSION['utilizadorid'] != NULL and $id_d!=$_SESSION['utilizadorid']) {
	...
	if ($result) {
		$result = $st -> fetch();
		if ($result[0] == TRUE) {
			$st = $db -> prepare('Update joingrupodeutilizadorestoutilizador set bloqueado=false where grupodeutilizadoresid=:id and :id_d=utilizadorid;');
			$st -> bindParam(':id', $id);
			$st -> bindParam(':id_d', $id_d);
			$result = $st -> execute();
			if ($result) {
				echo json_encode("Utilizador com id $id_d desbloqueado no grupo com id $id.");
